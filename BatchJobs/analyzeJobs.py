...
def Write_Preamble(f):
	...
	f.write(numNodes);
	wallTime = "#PBS -l walltime=03:00:00\n";
	f.write(wallTime);
	f.write("#PBS -N myjob\n");
	f.write("#PBS -j oe\n");
...
def DeployJob(fileName):
	...
	os.system(cmd);
        ...
        cmd = 'cd /users/j/b/jbongard/tauVACC/BatchJobs \n';
...
for a in range(1,2+1):
	...
	for r in range(1,100+1):
		...
		fileName = "../SavedFiles/writeout"+str(100*a+r)+".txt";
		#print fileName;
		f = open(fileName,'r');
		for line in f.readlines():
			a=a+0;
		line = line.split();
		runs.append(float(line[3]));
		f.close();

	distances.append(runs);

print 'Averages...'
for runs in distances:
