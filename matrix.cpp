...
void MATRIX::AddColumn(double val) {

	MATRIX *temp = new MATRIX(length,width+1,0.0);
...
void MATRIX::AddRow(double val) {
        ...
        MATRIX *temp = new MATRIX(length+1,width,0.0);
...
int  MATRIX::FirstRowWithSum(double val) {

	int rowIndex = 0;
	int found = false;

	while ( (!found) && (rowIndex<length) ) {

		if ( SumOfRow(rowIndex) == val )
...
int  MATRIX::NumOccurancesOf(double val) {
...
int MATRIX::Row_AllZeros(int i) {

	int j=0;
	int allZeros = true; 

	while ( (allZeros) && (j<width) )
...
int  MATRIX::ValFoundOffTheDiagonal(double val) {
...
void MATRIX::ValFoundOffTheDiagonal(int *rowIndex, int *columnIndex, double val) {
...
int  MATRIX::ValuesDifferentFrom(double val) {

	int numberOfDifferentValuesFound = 0;
	...
	for (int i=0; i<length; i++)
		...
		for (int j=0; j<width; j++)
			...
			if ( Get(i,j) != val )
				...
				numberOfDifferentValuesFound++;

	return( numberOfDifferentValuesFound ); 
}
...
void MATRIX::Write(ofstream *outFile) {
	...
	for (int i=0;i<length;i++) {
		...
		for (int j=0;j<width;j++)
			...
			(*outFile) << setprecision(10) << Get(i,j) << " ";
